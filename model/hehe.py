...
PATH_SUMMARY = 'E:\\WorkSpace\\WorkSpace\\TrainingLogs\\BVAE\\CIFAR10'
PATH_SNAPSHOT = 'E:\\WorkSpace\\WorkSpace\\SavedModels\\BVAE\\CIFAR10'
...
class BinaryEncodingCVAE(object):
    ...
    def _build_loss_2(self):
        ...
        corr = tf.matmul(self.nets[4], self.nets[5], transpose_a=True)
    ...
    def _get_optimizer(self):
        return tf.train.AdamOptimizer(learning_rate=0.000035).minimize(self.loss, global_step=self.g_step)
    ...
    def run_training(self, max_iter=15000, summary_path=PATH_SUMMARY, snapshot_path=PATH_SNAPSHOT, batch_reader=None):
        ...
        for i in range(max_iter):
            ...
            _, step_count, this_loss, summaries, _ = self.sess.run(
            ...
            del this_batch, batch_feature, batch_label, summaries
            # j = (i + 1) % 590
            # if j == 0:
            #     j = 590
            # if i >= 0:
    ...
    def _eval_loop(self, train_code, train_label, batch_reader, num=10):
        code = 0
        cls = 0
        for i in range(num):
    ...
    def run_extraction(self, batch_reader=None):
        if batch_reader is None:
            batch_reader = self.batch_reader
        assert batch_reader is not None
...
if __name__ == '__main__':
    ...
    re_file = 'E:\\WorkSpace\\WorkSpace\\SavedModels\\BVAE\\CIFAR10\\YMModel-14000'
    model = BinaryEncodingCVAE(code_length=32, feature_length=4096, label_num=10, sess=this_session, latent_size=1024,
                               batch_reader=reader, restore_file=re_file)

    model.run_training()
    # model.run_extraction()
